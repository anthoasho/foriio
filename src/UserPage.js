...
function ProfileCard(props){
  const [data, loading] = useFetch(`https://api.foriio.com/api/v1/users/${props.match.params.name}/profile`)
   if(!loading){
     let {profile} = data
       ...
       <div className = "profile-card">
         ...
         <div className="profile-information">
          ...
          {profile.bio &&<div className="profile-bio">
           <h3 className="profile-bio-title mobile-margin"> About me</h3>
...
function ProfileProjects(props){
  const [data, loading] = useFetch(`https://api.foriio.com/api/v1/users/${props.match.params.name}/works`);
  if(!loading){
    return(
