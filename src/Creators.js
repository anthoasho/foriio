...
function Creators(props) {
  const [data, loading] = useFetch(
      "https://api.foriio.com/api/v1/promoted/users"
    );
  let users
  if(!loading){
...
function SingleUser(props){
  ...
  return(
    <div className="list-user">
      <div className="avatar-container"> <Link to={`/user/${props.screen_name}`}> <Image className="list-user-avatar circle" src={props.avatar} /> </Link></div>
      <div className="meta">
      <Link to={`/user/${props.screen_name}`}> <Title className="username">{props.name}</Title> </Link>
      <Title className="profession">{props.profession}</Title>
      <Title className="screenname">{props.screen_name}</Title>
      </div>
